...
""" 
Se houver uma letra b no final da string de modo, significa que o fluxo deve
ser aberto em modo binário.
...
""" 
O modo de texto é o comportamento padrão assumido quando nenhum especificador
de modo binário/texto é usado.

A abertura bem-sucedida de um arquivo definirá a posição atual do arquivo
(o cabeçote virtual de leitura/gravação) antes do primeiro byte do arquivo se o
modo não for "a" e após o último byte do arquivo se o modo estiver definido como "a".
"""
...
""" 
EXTRA
Você também pode abrir um arquivo para sua criação exclusiva. Você pode fazer
isso usando o "x" open mode.
...
""" 
Em programação, "fluxos" referem-se a uma maneira de lidar com a entrada e saída
de dados.
São canais de comunicação através dos quais você pode enviar e receber informações.
"""


""" Dissemos anteriormente que qualquer operação de stream deve ser precedida pela
invocação da função open().
Existem três exceções bem definidas à regra.

Quando nosso programa começa, os três streams já estão abertos e não requerem
nenhum preparo extra.
Além do mais, seu programa pode usar esses fluxos explicitamente se você importa
o módulo sys, porque é onde é colocada a declaração dos três fluxos.
...
stdin (como entrada padrão)
    o fluxo stdin é normalmente associado ao teclado, pré-aberto para leitura e
    considerado a principal fonte de dados para os programas em execução;
    a conhecida função input() lê dados de stdin por padrão.

sys.stdout
stdout (como saída padrão)
    o fluxo stdout é normalmente associado à tela, pré-aberta para escrita,
    considerada o alvo principal para saída de dados pelo programa em execução;
    a conhecida função print() envia os dados para o fluxo stdout.

sys.stderr
stderr (como saída de erro padrão)
    o fluxo stderr normalmente está associado à tela, pré-aberta para escrita,
    considerada o local principal para onde o programa em execução deve enviar
    informações sobre os erros encontrados durante seu trabalho; a separação do
    stdout (resultados úteis produzidos pelo programa) do stderr (mensagens de
    erro, inegavelmente úteis, mas que não fornecem resultados) dá a possibilidade
    de redirecionar estes dois tipos de informação para os diferentes alvos.
"""
...
""" A última operação executada em um stream (isso não inclui os streams stdin,
stdout e stderr, que não exigem isso) deve ser "close".

Essa ação é executada por um método invocado de dentro do objeto de fluxo aberto:
stream.close()."""


""" 
a maioria dos desenvolvedores acredita que a função close() sempre é bem-sucedida
e, portanto, não há necessidade de verificar se ela executou sua tarefa corretamente.

Esta crença é apenas parcialmente justificada. Se o fluxo foi aberto para escrita
e depois uma série de operações de escrita foram executadas, pode acontecer que os
dados enviados ao fluxo ainda não tenham sido transferidos para o dispositivo
físico (devido a um mecanismo chamado cache ou buffer).

Como o fechamento do fluxo força os buffers a liberá-los, pode ser que as
liberações falhem e, portanto, o close() falhe também.
"""


""" Diagnosticando problemas de fluxo (stream) """


"""O objeto IOError é equipado com uma propriedade chamada errno (o nome vem da
frase número do erro) e você pode acessá-lo da seguinte forma:"""
...
O erro ocorre quando você tenta, por exemplo, abrir um arquivo com o atributo
somente leitura para escrita.
...
O erro ocorre quando você tenta, por exemplo, renomear um arquivo com seu nome
anterior.

errno.EFBIG → Arquivo muito grande
O erro ocorre ao tentar criar um arquivo maior que o máximo permitido pelo
sistema operacional.

errno.EISDIR → É um diretório
O erro ocorre quando você tenta tratar um nome de diretório como o nome de
um arquivo comum.

errno.EMFILE → Muitos arquivos abertos
O erro ocorre quando você tenta abrir simultaneamente mais fluxos do que o
aceitável para o seu sistema operacional.
